...
Future uploadMultipleImages(File img) async {
  ...
  var uri = Uri.parse("http://192.168.15.124/easy/upload_property_images.php?Ads_Id="+adsId.toString()+"&Client_ID="+89.toString()); //sending post request with header data
...
Future uploadCarsImages(File img) async {
  ...
  var uri = Uri.parse("http://192.168.15.124/easy/upload_cars_images.php?Ads_Id="+carAdsId.toString()+"&Client_ID="+89.toString()); //sending post request with header data
  http.MultipartRequest request = http.MultipartRequest('POST', uri,);
  List<MultipartFile> newList = [];
  File imageFile = File(img.path);
  var stream = http.ByteStream(imageFile.openRead())..cast();
